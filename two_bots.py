...
guess2 = np.array([0.31001,0.31001,math.pi/2,math.pi/2])
guess3 = np.array([0.31001,.31001,.31001,0,math.pi/2,math.pi/2])
...
for j in range(1+len(long_path)) :
    if j != len(long_path) :
        ...
        solution =  minimize(velocity_of_VS_vertices, guess2,method='SLSQP', bounds = ((lower_bound[0],upper_bound[0][j]),(lower_bound[1],abs(upper_bound[1][j])),(0,math.pi),(0,math.pi)))
        ...
        solution.x[1+NUM_ROBOTS]= -1*solution.x[1+NUM_ROBOTS]
...
tck1x = interpolate.splrep(xnew,bot1x,s=0,k=1)
tck1y = interpolate.splrep(xnew,bot1y,s=0,k=1)
tck2x = interpolate.splrep(xnew,bot2x,s=0,k=1)
tck2y = interpolate.splrep(xnew,bot2y,s=0,k=1)
