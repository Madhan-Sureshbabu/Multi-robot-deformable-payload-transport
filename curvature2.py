...
def bounds_calc(hom_matrix,j):
	...
	for obstacle in mf.circularObstacles :
		...
		if obstacle[2]**2 - obs_centre_x**2 > 0 :
			...
			if y_intersection - mf.bot_rad < 1.5 :#and y_intersection - mf.bot_rad > 0.2:
				upper_bound[1][j] = y_intersection - mf.bot_rad
...
def plan_path():
	...
	long_path,vel_x,vel_y,k0 = mf.curvefit(pathList,2*pathList.shape[0])
...
guess2 = np.array([0.011,0])
axis_guess = np.array([0,0,0])


# j = 3
while path_feasible == False :
	plan_path()
	path_feasible= True
	for j in range(1+len(long_path)) :
		if j != len(long_path) :
			...
			solution = least_squares(velocity_of_VS_vertices2,guess2,'3-point',bounds=([0.01, 0 ], [ upper_bound[1][j],math.pi ]))
...
# Bot_path[0],a ,b ,c = mf.curvefit(Bot_path[0],len(Bot_path[0]))
# Bot_path[1],a ,b ,c = mf.curvefit(Bot_path[1],len(Bot_path[1]))
